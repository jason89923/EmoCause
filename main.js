...
async function main() {
  ...
  app.post('/API/nextPage', ensureAuthenticated, async (req, res) => {
    ...
    if (tail === undefined) {
      ...
      var current_page = current_user.history.length;
    }

    if (record === undefined) {
      if (current_user.history.length === 0) {
        // 按照使用者作答的次數排序後，取出第一個(最少)
        ...
        await user_collection.updateOne({ sub: user_id }, { $addToSet: { history: new_page.doc_id }, $set: { current_page: current_page } });
      } else {
        // 重新整理
        var new_page = await topic_collection.findOne({ doc_id: current_user.history[current_page - 1] });
        const result = await topic_collection.aggregate([
          { $match: { doc_id: current_user.history[current_page - 1] } },
          {
            $project: {
              label_item: {
                $arrayElemAt: [
        ...
        answer = result[0]?.label_item;
        await user_collection.updateOne({ sub: user_id }, { $set: { current_page: current_page } });
      }
    }
    else {
      await topic_collection.updateOne(
        { doc_id: current_user.history[current_user.current_page - 1] },
        { $pull: { label: { sub: user_id } } }
      );
  
      await topic_collection.updateOne(
        { doc_id: current_user.history[current_user.current_page - 1] },
        { $push: { label: { sub: user_id, selections: record } } }
      );
      
      current_page += 1;
      if (current_page > current_user.history.length) {
        var new_page = (await topic_collection.aggregate([
          {
            $match: { doc_id: { $nin: current_user.history } }
          },
      ...
      else if ( current_page <= current_user.history.length && current_page > 0 ) {
        var new_page = await topic_collection.findOne({ doc_id: current_user.history[current_page - 1] });
        const result = await topic_collection.aggregate([
          { $match: { doc_id: current_user.history[current_page - 1] } },
          {
    ...
    res.json({ current_page: current_page, answer: answer, total_page: total_page, docs: new_page });
    // res.json({ current_page: current_page, total_page: total_page, docs: new_page });
  });

  app.post('/API/prevPage', ensureAuthenticated, async (req, res) => {
    // 總頁數、下一頁題目內容
    // 確認使用者是否存在
    const user_id = req.body.uuid;
    const record = req.body.selections;
    ...
    const total_page = (await topic_collection.find({}).toArray()).length;
    const current_user = (await user_collection.findOne({ sub: user_id }));
    let current_page = current_user.current_page;

    await topic_collection.updateOne(
      { doc_id: current_user.history[current_page - 1] },
      { $pull: { label: { sub: user_id } } }
    );
    ...
    const answer = result[0]?.label_item;
    await user_collection.updateOne({ sub: user_id }, { $set: { current_page: current_page } });
    res.json({ current_page: current_page, answer: answer, total_page: total_page, docs: new_page });
  });
